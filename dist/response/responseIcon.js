 ...
 */
export default function responseIcon(id, req, sharp, iconMap, fallback) {
    return __awaiter(this, void 0, void 0, function* () {
        var _a;
        const { searchParams } = new URL(req.url);
        const size = Math.min(Math.max(parseInt(searchParams.get('size') || String(DEFAULT_SIZE)), 16), 1024);
        const bg = normalizeColor(searchParams.get('bg') || DEFAULT_BG);
        const color = normalizeColor(searchParams.get('color') || DEFAULT_ICON_COLOR);
        const Icon = (_a = iconMap[id]) !== null && _a !== void 0 ? _a : fallback;
        if (!Icon) {
            return new Response(null, { status: 404 });
        }
        const { renderToStaticMarkup } = yield import('react-dom/server');
        const iconMarkup = renderToStaticMarkup(Icon({ fill: color, stroke: color, style: { color } }));
  ...
  <svg x="${padding}" y="${padding}" width="${iconSize}" height="${iconSize}" viewBox="${viewBox}" fill="${color}" stroke="${color}">
    ${innerContent}
        ...
        const png = yield sharp(Buffer.from(fullSvg))
            .resize(size, size)
            .png()
            .toBuffer();
